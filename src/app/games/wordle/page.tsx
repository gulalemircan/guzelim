                ...
                >
                  {Array.from({ length: wordLength }).map((_, colIndex) => {
                    ...
                    if (rowIndex < guesses.length) { 
                      ...
                      else bgColor = "bg-black/5 text-text/40 border-black/10";
                ...
                {KEYBOARD_ROWS.map((row, i) => (
                  <div key={i} className="flex justify-center gap-1">
                    {row.map(key => {
                      ...
                      if (status === "absent") keyBg = "bg-black/5 text-text/40 border-transparent opacity-50";
