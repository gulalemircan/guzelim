...
export default function TripRoomPage() {
  ...
  const handleBunnyClick = () => {
    playSound("click");
    setBunnyBounce(true);
    setTimeout(() => setBunnyBounce(false), 500);
  };
    ...
    <main className={`min-h-screen w-full flex flex-col relative overflow-hidden transition-colors duration-1000 ${roomMode === 'trip' ? 'bg-[#0f172a]' : 'bg-[#F4EED3]'}`}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center perspective-[1200px] pt-16 pb-10 overflow-x-hidden">
              ...
              <div className="mt-3 w-full flex flex-col items-center gap-2">
                 {currentUser === "Efsun" && (
                   <div className={`w-full p-2.5 rounded-xl border backdrop-blur-sm transition-colors ${roomMode === 'trip' ? 'bg-white/5 border-white/10' : 'bg-card/50 border-primary/20'}`}>
                      <input type="text" value={newReelUrl} onChange={(e) => setNewReelUrl(e.target.value)} placeholder="Tiktok/Reels Linki..." className="w-full bg-transparent border-b border-current/20 pb-1 mb-1.5 text-xs outline-none text-current" />
                      <input type="text" value={newReelNote} onChange={(e) => setNewReelNote(e.target.value)} placeholder="İğneleyici notun..." className="w-full bg-transparent border-b border-current/20 pb-1 mb-1.5 text-xs outline-none text-current" />
                      <button onClick={handleAddReel} className="w-full py-1 bg-[#8B5A2B] text-white rounded-md text-[10px] font-bold uppercase tracking-widest hover:bg-[#6b4420] transition-colors">Panoya İğnele</button>
                   </div>
                 )}

                 <div className="w-full flex justify-end">
                    <div className="relative">
                      <button onClick={() => setConfirmTrash(true)} className={`w-12 h-14 rounded-b-xl rounded-t-sm border-2 flex flex-col items-center justify-start pt-1 transition-all hover:scale-105 ${roomMode === 'trip' ? 'bg-[#1e293b] border-white/20' : 'bg-[#D4A373] border-[#8B5A2B]'}`}>
                         <div className={`w-12 h-2 absolute -top-2 rounded-sm ${roomMode === 'trip' ? 'bg-white/20' : 'bg-[#8B5A2B]'}`}></div>
                         <span className="text-lg mt-1 opacity-50">🗑️</span>
                      </button>
        ...
        <div className="w-full max-w-2xl h-24 bg-[#5C4033] border-t-8 border-[#3E2723] rounded-t-2xl shadow-2xl relative mx-auto mt-4 flex items-center justify-around px-8 z-30">
            ...
            {/* 1. Stres Tavşanı 🐰 */}
            <div onClick={handleBunnyClick} className={`cursor-pointer flex flex-col items-center transition-transform duration-200 ${bunnyBounce ? 'scale-125 -translate-y-4' : 'hover:scale-110'}`} title="Hırs Çıkarma Tavşanı">
               <span className="text-3xl filter drop-shadow-md">🐰</span>
               <span className="text-[9px] text-white/70 font-bold uppercase tracking-wider bg-black/30 px-1.5 py-0.5 rounded-full mt-1">Stres Tavşanı</span>
            </div>

            {/* 2. Lilyum Vazosu 🌸 */}
            <div className="flex flex-col items-center" title="Lilyum Vazosu">
               <span className="text-3xl filter drop-shadow-md">{roomMode === 'trip' ? '🥀' : '🌸'}</span>
               <span className="text-[9px] text-white/70 font-bold uppercase tracking-wider bg-black/30 px-1.5 py-0.5 rounded-full mt-1">
                 {roomMode === 'trip' ? 'Solgun Vazo' : 'Canlı Lilyumlar'}
               </span>
            </div>

            {/* 3. Trip Kahvesi ☕ */}
            <div onClick={handleCoffeeClick} className="cursor-pointer flex flex-col items-center relative group" title="Trip Kahvesi">
               {roomMode === 'trip' && coffeeClickCount < 5 && (
                  <div className="absolute -top-6 text-xl animate-bounce">☁️</div>
               )}
               <span className="text-3xl filter drop-shadow-md">☕</span>
               <span className="text-[9px] text-white/70 font-bold uppercase tracking-wider bg-black/30 px-1.5 py-0.5 rounded-full mt-1">
                 {coffeeClickCount >= 5 ? 'Kahve Soğudu' : 'Dumanlı Kahve'}
               </span>
            </div>
            ...
            <div className="flex flex-col items-center">
               <div className={`w-12 h-10 bg-[#8B5A2B] border-2 border-[#3E2723] rounded-md flex items-center justify-center shadow-lg transition-all duration-700 ${roomMode === 'trip' ? 'rotate-x-90 opacity-40 scale-95' : 'rotate-x-0 scale-100 bg-white p-1'}`}>
                  {roomMode === 'peace' ? (
                     <div className="w-full h-full bg-pink-100 rounded text-[8px] flex items-center justify-center font-bold text-pink-700">E & E</div>
                  ) : (
