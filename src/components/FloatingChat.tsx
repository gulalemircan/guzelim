...
export default function FloatingChat() {
  ...
  // Arka planda motoru sessizce kur (Ekrana yazı yazmaz)
  useEffect(() => {
  ...
  useEffect(() => {
    if (isOpen) window.dispatchEvent(new CustomEvent("chat-opened"));
    else window.dispatchEvent(new CustomEvent("chat-closed"));
  }, [isOpen]);
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, (payload) => {
        ...
        if (newMsg.text.startsWith("🔊 Sesli Mesaj: ")) {
          ...
          if (foundMeme && foundMeme.url) {
            ...
            audio.volume = foundMeme.volume ?? 1.0;
  ...
  const handleEnableNotifications = async () => {
    try {
      ...
      if (permission === 'granted') {
        ...
        if (!vapidKey) return alert("VAPID Key eksik!");
        ...
        const savedName = localStorage.getItem("myName") || "Efsun";
        const { error } = await supabase.from('push_subscriptions').insert([{ 
           user_name: savedName, 
           subscription: JSON.parse(JSON.stringify(sub)) 
        }]);

        if (error) alert("Veritabanı Hatası: " + error.message);
        else alert("✅ Harika! Bildirimler başarıyla açıldı.");
        
      } else {
        alert("Bildirim izni vermedin :(");
      }
    } catch (err) {
      alert("Hata oluştu: Lütfen cihazınızın şarjını ve güç tasarrufu modunu kontrol edin.");
    }
  };
  ...
  const handleSend = async (customText?: string) => {
    ...
    if (textToSend.toLowerCase().includes("peki") || textToSend.toLowerCase().includes("sen bilirsin")) {
      setErrorAnim(true);
    ...
    if (!error) {
      ...
      let notificationText = textToSend.startsWith("🔊 Sesli Mesaj: ") ? "Sana bir sesli mesaj gönderdi 🎵" : textToSend;
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      {isOpen && (
        <div className="bg-card border border-primary/20 rounded-[32px] shadow-2xl flex flex-col overflow-hidden w-[calc(100vw-3rem)] sm:w-[350px] h-[65dvh] sm:h-[500px] mb-2 sm:mb-4 animate-in slide-in-from-bottom-5">
          ...
          <div className="bg-primary/10 border-b border-primary/20 p-4 flex flex-col gap-3 backdrop-blur-md">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                {/* ZİL BUTONU - TEMİZ VE ŞIK */}
                <button onClick={handleEnableNotifications} className="w-8 h-8 flex items-center justify-center bg-blue-500 text-white rounded-full hover:bg-blue-600 transition-colors shadow-lg active:scale-95" title="Bildirimleri Aç">🔔</button>
                <button onClick={() => { localStorage.clear(); sessionStorage.clear(); window.location.replace('/'); }} className="text-[10px] bg-red-500/20 text-red-500 px-2 py-1 rounded-lg font-bold hover:bg-red-500 hover:text-white transition-colors">Çıkış</button>
                <button onClick={() => setIsOpen(false)} className="w-8 h-8 flex items-center justify-center bg-card rounded-full text-primary hover:bg-primary hover:text-background transition-colors font-bold shadow-sm">✕</button>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 relative">
            ...
            {errorAnim && <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-red-600 text-white px-4 py-2 rounded-full text-[10px] font-bold tracking-widest uppercase animate-bounce shadow-xl whitespace-nowrap">🚨 Yasaklı Kelime! 🚨</div>}
          </div>
          ...
          {showMemeMenu && (
            <div className="flex gap-2 p-2 bg-background/90 border-t border-primary/20 overflow-x-auto">
              {memes.length === 0 ? (
                ...
                memes.map(meme => <button key={meme.id} onClick={() => sendMeme(meme.name)} className="bg-primary/20 hover:bg-primary/40 text-primary border border-primary/30 px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-colors">{meme.name} 🎵</button>)
              )}
            </div>
          )}
      ...
      {!isOpen && <button onClick={() => setIsOpen(true)} className="w-16 h-16 bg-primary text-background rounded-full flex items-center justify-center text-3xl shadow-[0_8px_30px_rgb(0,0,0,0.4)] hover:scale-110 active:scale-95 transition-all duration-300 animate-bounce">💬</button>}
    </div>
  );
}
