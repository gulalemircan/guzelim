...
export default function GlobalMusicPlayer() {
  ...
  // YENİ: Chat'in açık olup olmadığını dinleyen state
  const [isChatOpen, setIsChatOpen] = useState(false); 
  const audioRef = useRef<HTMLAudioElement | null>(null);

  // YENİ: Chat açıldığında veya kapandığında bunu yakalayan kulak
  useEffect(() => {
    const handleChatOpen = () => {
      setIsChatOpen(true);
      setIsExpanded(false); // Chat açılınca müzik barı açıksa onu da zorla kapat
    };
    const handleChatClose = () => setIsChatOpen(false);

    window.addEventListener("chat-opened", handleChatOpen);
    window.addEventListener("chat-closed", handleChatClose);

    return () => {
      window.removeEventListener("chat-opened", handleChatOpen);
      window.removeEventListener("chat-closed", handleChatClose);
    };
  }, []);
  ...
  return (
    // YENİ: isChatOpen true ise plağı küçültüp (scale-0) görünmez yapıyoruz ve tıklanmasını (pointer-events-none) engelliyoruz
    <div 
      className={`fixed bottom-6 left-6 z-[100] flex items-center transition-all duration-500 ease-in-out ${
        isChatOpen ? "opacity-0 scale-50 pointer-events-none translate-y-10" : "opacity-100 scale-100 translate-y-0"
      }`}
    >
